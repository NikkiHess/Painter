package com.nickkhess.painter.utils;
...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public boolean containsOnly(Material type) {
		for (Block block : this) {
			if (block.getType() != type) return false;
		}
		return true;
	}
	...
	public Cuboid move(int offX, int offY, int offZ) {
		return new Cuboid(worldName, x1 + offX, y1 + offY, z1 + offZ, x2 + offX, y2 + offY, z2 + offZ);
	}
	...
	public Cuboid copy(int offX, int offY, int offZ) {
		for(Block b : getBlocks()) {
			Block o = b.getLocation().add(offX, offY, offZ).getBlock();
			if(o.getType() != b.getType())
				o.setType(b.getType());
		}
		return move(offX, offY, offZ);
	}
